...
/**
 * Order book should handle placing a FOK order when the book is empty.
 * Order should be unmatched since there are no other orders to match with and
 * should then be cancelled.
 */
TEST(VectorOrderBookTest, BookShouldInsertUnmatchedOrders2) {
    // Create a FOK order.
    Order order = Order::askLimit(OrderType::FillOrKill, 200, 100, 1, 1, 1);
    // Create a messenger for the order book.
    OrderBookReceiver result_receiver;
    // Start up a worker waiting for results.
    std::thread t1 {&OrderBookReceiver::start, &result_receiver};
    ...
    result_receiver.stop();
    t1.join();
    // Book should not contain the order since it is a FOK order - FOK orders are never inserted into the book.
...
/**
 * Order book should handle placing GTC orders on the opposite side of the book.
 * Orders should not be matched since they are not at matchable prices.
 */
TEST(VectorOrderBookTest, BookShouldInsertUnmatchedOrders4) {
...
/**
 * Order book should be able to handle a FOK order that is matches with GTC orders at multiple
...
TEST(VectorOrderBookTest, BookShouldMatchOrders4) {
    ...
    Order order3 = Order::bidLimit(OrderType::FillOrKill, 190, 150, 3, 3, 1);
    // Create a messenger for the order book.
    OrderBookReceiver result_receiver;
    // Start up a worker waiting for results.
    std::thread t1 {&OrderBookReceiver::start, &result_receiver};
    ...
    book.placeOrder(order3);
    // Book should not contain the FOK order - FOK orders are never inserted into the book.
    ASSERT_FALSE(book.hasOrder(3));
