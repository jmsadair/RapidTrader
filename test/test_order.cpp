...
TEST(OrderTest, DifferentOrdersNotEqual)
{
    ...
    const auto order1 = Order::bidLimit(order1_type, order1_quantity, order1_price, order1_id, order1_uid, order1_symbol_id);
    const auto order2 = Order::bidLimit(order2_type, order2_quantity, order2_price, order2_id, order2_uid, order2_symbol_id);
...
TEST(OrderTest, SameOrdersEqual)
{
    ...
    const auto order1_symbol_id = 1;
    const auto order1 = Order::bidLimit(order1_type, order1_quantity, order1_price, order1_id, order1_uid, order1_symbol_id);
    const auto order2 = Order::bidLimit(order1_type, order1_quantity, order1_price, order1_id, order1_uid, order1_symbol_id);
