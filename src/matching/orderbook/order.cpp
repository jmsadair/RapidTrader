    ...
    uint64_t order_id, uint32_t symbol_id, uint64_t price, uint64_t stop_price, uint64_t quantity, OrderTimeInForce time_in_force)
{
    ...
    assert(stop_price > 0 && "Stop Price must be positive!");
    assert(quantity > 0 && "Quantity must be positive!");
    return Order{OrderType::TrailingStopLimit, OrderSide::Ask, time_in_force, symbol_id, price, stop_price, quantity, order_id};
}
    ...
    uint64_t order_id, uint32_t symbol_id, uint64_t price, uint64_t stop_price, uint64_t quantity, OrderTimeInForce time_in_force)
{
    ...
    assert(stop_price > 0 && "Stop Price must be positive!");
    assert(quantity > 0 && "Quantity must be positive!");
    return Order{OrderType::TrailingStopLimit, OrderSide::Bid, time_in_force, symbol_id, price, stop_price, quantity, order_id};
}
...
{
    // Stop order price should always be positive if the order is a restart order.
    // Price should always be positive for limit and restart limit orders.
    if (type == OrderType::Limit)
        assert(price > 0 && "Limit orders must have a positive price!");
    if (type == OrderType::Stop || type == OrderType::TrailingStop)
        assert(stop_price > 0 && "Stop and trailing stop orders must have a positive restart price!");
    if (type == OrderType::StopLimit || type == OrderType::TrailingStopLimit)
        assert(stop_price > 0 && "Stop limit and trailing stop limit orders must have a positive restart price!");
    // Market orders and restart orders can only have time in force FOK or IOC.
    if (type == OrderType::Market || type == OrderType::Stop || type == OrderType::TrailingStop)
        assert(time_in_force != OrderTimeInForce::GTC && "Market and restart orders cannot have GTC time in force!");
